...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof sinqi, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( sinqi.length, 4, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns a reference to the workspace array', function test( t ) {
	...
	N = 8;
	workspace = new Float32Array( ( 3*N ) + 34 );
	out = sinqi( N, workspace, 1, 0 );

	t.strictEqual( out, workspace, 'same reference' );
	t.end();
});

tape( 'the function correctly initializes cosine values (small sequence lengths)', function test( t ) {
	var workspace;
	var expected;
	var lengths;
	var offsets;
...
tape( 'the function correctly initializes cosine values (medium sequence lengths)', function test( t ) {
	var workspace;
	var expected;
	var lengths;
	var offsets;
...
tape( 'the function correctly initializes cosine values (large sequence lengths)', function test( t ) {
	var workspace;
	var expected;
	var lengths;
	var offsets;
...
tape( 'the function correctly handles stride and offset parameters', function test( t ) {
	...
	stride = 2;
	offset = 3;
	workspace = new Float32Array( offset + ( ( ( 3*N ) + 34 ) * stride ) );

	sinqi( N, workspace, stride, offset );

	t.strictEqual( workspace[ offset + ( 3*N * stride ) ], N, 'returns expected value' );

	nf = workspace[ offset + ( ( ( 3*N ) + 1 ) * stride ) ];
	t.strictEqual( nf, 2, 'returns expected value' );
	t.strictEqual( workspace[ offset + ( ( ( 3*N ) + 2 ) * stride ) ], 2, 'returns expected value' );
	t.strictEqual( workspace[ offset + ( ( ( 3*N ) + 3 ) * stride ) ], 4, 'returns expected value' );

	expected = new Float32Array( ( 3*N ) + 34 );
